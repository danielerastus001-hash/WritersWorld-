...
def _self_test_generator(admin_id):
    ...
    yield result("Self-Test Suite", "start", "Starting comprehensive test run...")

    try:
        yield result("Create test users", "running")
        try:
            ...
            db.session.commit()
            yield result("Create test users", "pass", f"{ua_username}, {ub_username}")
        except Exception as e:
            db.session.rollback()
            yield result("Create test users", "fail", str(e))
            raise

        yield result("Create & publish story", "running")
        try:
            ...
            db.session.add(story)
            db.session.commit()
            yield result("Create & publish story", "pass", f"story_id={story.id}")
        except Exception as e:
            db.session.rollback()
            yield result("Create & publish story", "fail", str(e))

        yield result("Like story", "running")
        try:
            like = Like(user_id=user_b.id, story_id=story.id)
            db.session.add(like)
            db.session.commit()
            ok = story.like_count() == 1
            yield result("Like story", "pass" if ok else "fail", f"like_count={story.like_count()}")
        except Exception as e:
            db.session.rollback()
            yield result("Like story", "fail", str(e))

        yield result("Comment on story", "running")
        try:
            ...
            yield result("Comment on story", "pass" if ok else "fail", f"comment_count={story.comment_count()}")
        except Exception as e:
            db.session.rollback()
            yield result("Comment on story", "fail", str(e))

        yield result("Track story view", "running")
        try:
            ...
            db.session.commit()
            ok = story.views == 1
            yield result("Track story view", "pass" if ok else "fail", f"views={story.views}")
        except Exception as e:
            db.session.rollback()
            yield result("Track story view", "fail", str(e))

        yield result("Create competition", "running")
        try:
            today = datetime.utcnow().date()
            ...
            yield result("Create competition", "pass" if ok else "fail", f"status={comp.status()}")
        except Exception as e:
            db.session.rollback()
            yield result("Create competition", "fail", str(e))

        yield result("Enter competition", "running")
        try:
            ...
            db.session.add(entry)
            db.session.commit()
            yield result("Enter competition", "pass", f"entry_id={entry.id}")
        except Exception as e:
            db.session.rollback()
            yield result("Enter competition", "fail", str(e))

        yield result("Submit peer review", "running")
        try:
            ...
            db.session.add(review)
            db.session.commit()
            yield result("Submit peer review", "pass", f"review_id={review.id}")
        except Exception as e:
            db.session.rollback()
            yield result("Submit peer review", "fail", str(e))

        yield result("Claim entry reward (first time)", "running")
        try:
            ...
            yield result("Claim entry reward (first time)", "pass" if ok else "fail",
                      f"before={gems_before}, after={gems_after}")
        except Exception as e:
            db.session.rollback()
            yield result("Claim entry reward (first time)", "fail", str(e))

        yield result("Duplicate reward claim blocked", "running")
        try:
            already = GemTransaction.query.filter_by(
                user_id=user_b.id, source='entry_read_reward'
            ).filter(GemTransaction.detail.like(f"entry:{entry.id}:%")).first()
            ok = already is not None
            yield result("Duplicate reward claim blocked", "pass" if ok else "fail")
        except Exception as e:
            yield result("Duplicate reward claim blocked", "fail", str(e))

        yield result("Admin submits expert review", "running")
        try:
            ...
            db.session.commit()
            yield result("Admin submits expert review", "pass", f"review_id={exp_review.id}")
        except Exception as e:
            db.session.rollback()
            yield result("Admin submits expert review", "fail", str(e))

        yield result("Expert review retrievable", "running")
        try:
            found = ExpertReview.query.filter_by(entry_id=entry.id).first()
            ok = found is not None and found.content != ""
            yield result("Expert review retrievable", "pass" if ok else "fail")
        except Exception as e:
            yield result("Expert review retrievable", "fail", str(e))

        yield result("Admin sends message", "running")
        try:
            msg1 = AdminMessage(admin_id=admin_id, user_id=user_a.id,
                               message="Self-test admin message", is_reply=False)
            db.session.add(msg1)
            db.session.commit()
            yield result("Admin sends message", "pass", f"msg_id={msg1.id}")
        except Exception as e:
            db.session.rollback()
            yield result("Admin sends message", "fail", str(e))

        yield result("Admin sees user's reply", "running")
        try:
            ...
            yield result("Admin sees user's reply", "pass" if ok else "fail",
                      f"found {len(found_messages)} messages, expected 2")
        except Exception as e:
            db.session.rollback()
            yield result("Admin sees user's reply", "fail", str(e))

        yield result("Create & delete announcement", "running")
        try:
            ...
            yield result("Create & delete announcement", "pass" if ok else "fail")
        except Exception as e:
            db.session.rollback()
            yield result("Create & delete announcement", "fail", str(e))

        yield result("Delete competition (cascades entries)", "running")
        try:
            comp_id = comp.id
            ...
            yield result("Delete competition (cascades entries)", "pass" if ok else "fail")
        except Exception as e:
            db.session.rollback()
            yield result("Delete competition (cascades entries)", "fail", str(e))

        yield result("Delete story (cascades likes/comments)", "running")
        try:
            story_id = story.id
            db.session.delete(story)
            db.session.commit()
            ...
            story = None
            ok = still_exists is None
            yield result("Delete story (cascades likes/comments)", "pass" if ok else "fail")
        except Exception as e:
            db.session.rollback()
            yield result("Delete story (cascades likes/comments)", "fail", str(e))
        ...
        try:
            ...
            db.session.commit()
            yield result("Cleanup", "pass", "All test data removed")
        except Exception as e:
            db.session.rollback()
            yield result("Cleanup", "fail", str(e))

    yield result("Self-Test Suite", "done", "All tests complete.")
...
def self_test_stream():
    ...
    admin_id = current_user.id
    def wrap():
        for r in _self_test_generator(admin_id):
            yield f"data: {_json.dumps(r)}\n\n"
    return Response(wrap(), mimetype='text/event-stream')

@app.route('/admin/self_test/run_sync', methods=['POST'])
@login_required
@admin_required
def self_test_sync():
    admin_id = current_user.id
    results = list(_self_test_generator(admin_id))
